...
//@Service
public class CapIQExpressFeedServiceImpl implements CapIQService{
	private static final Logger log = LoggerFactory.getLogger(CapIQExpressFeedServiceImpl.class);
	...
	public PriceHistory getHistoricalData(CompanyInputRecord input) throws ResponseParserException, CapIQRequestException {			
		...
		for (CSVRecord record : records) {
			if(record.get(0).equalsIgnoreCase(input.getTicker()) && record.size() >= 7 && list.contains(record.get(2))){
				...
				hv.setValue(value);
				hv.setDate(DateUtil.toDate(record.get(5), "yyyy-MM-dd hh:mm:ss"));
	...
	public Holders getHolderDetails(CompanyInputRecord input) throws ResponseParserException, CapIQRequestException {
		...
		for (CSVRecord record : records) {
			...
			holder.setShares(Long.parseLong(record.get(3)));
			holder.setPercent(Double.parseDouble(record.get(4)));
	...
	public static void main(String[] args) throws ResponseParserException, CapIQRequestException{
		CapIQExpressFeedServiceImpl t = new CapIQExpressFeedServiceImpl();
